...
try {
  ...
  await page.route("**/api/billing/checkout", async (route) => {
    ...
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ url: "/app/brand?t=" + Date.now() + "#plan" }) });
  });
  ...
  ok("deleting the account starts by emailing a code, not by typing a word", (await page.locator("[data-test=delete]").getByRole("button", { name: /email me a code/i }).count()) === 1);
  ...
  ok("the send card comes first and shows the client link before anything is sent", await aside.evaluate((el) => { const card = el.querySelector("[data-test=send-card]"); const tabs = el.querySelector("[role=tablist]"); return Boolean(card && tabs) && card.compareDocumentPosition(tabs) === Node.DOCUMENT_POSITION_FOLLOWING; }) && /Not live yet/.test(await aside.locator("[data-test=link-row]").innerText()) && (await aside.locator("[data-test=publish-link]").count()) === 1);
  ...
  ok("options open with the look and four closed groups that each say what is set", (await aside.locator("[data-test^=opt-]").count()) === 4 && (await aside.locator("[data-test=opt-money]").innerText()).includes("no tax") && (await aside.locator("[data-test=opt-access]").innerText()).includes("Anyone with the link") && (await aside.locator("#taxRate").count()) === 0);
  await aside.locator("[data-test=opt-after]").getByRole("button", { name: /after they sign/i }).click();
  ...
  await aside.locator("[data-test=send-button]").click();
  const dlg = page.getByRole("dialog", { name: "Send proposal" });
  ok("the send dialog lists both recipients and says who gets the email", (await dlg.locator("input[type=email]").count()) === 2 && /cfo@bramble.example and ops@bramble.example/.test(await dlg.innerText()) && (await dlg.locator("[title^='http']").count()) === 1);
  ...
  await page.fill("#pricing-tax-rate", "13");
  await page.fill("#pricing-tax-label", "HST");
  const firstRow = page.getByRole("button", { name: /Design and build/ }).first();
  if ((await firstRow.getAttribute("aria-expanded")) !== "true") await firstRow.click();
  ok("one 13 % rate set under Pricing applies to every line: (10,000 + 1,200) x 1.13", (await total()) === money(1265600, "USD"), await total());
  ok("in-document preview shows the tax label HST", (await page.locator(".bn-editor").getByText("HST", { exact: true }).count()) > 0);
  const taxable = page.locator("label", { hasText: /^Taxable/ }).first();
  await taxable.click();
  await page.waitForTimeout(150);
  ok("switching one line to not taxable removes tax there only: 10,000 + 1,356", (await total()) === money(1135600, "USD"), await total());
  await taxable.click();
  await page.waitForTimeout(150);
  ok("switching it back returns to the proposal rate", (await total()) === money(1265600, "USD"), await total());
  await tabs(/options/i);
  await page.locator("[data-test=opt-money]").getByRole("button", { name: /money/i }).click();
  ok("the Money group under Options shows the same rate", (await page.locator("#taxRate").inputValue()) === "13.00");
  await page.fill("#taxRate", "");
  await tabs(/pricing/i);
  ok("removing the rate leaves no tax: 11,200", (await total()) === money(1120000, "USD"), await total());
  ok("no tax rows in the preview when the rate is zero", (await page.locator(".bn-editor").getByText("Subtotal").count()) === 0);
  await page.fill("#pricing-tax-rate", "13");
  await page.waitForTimeout(150);
  ok("typing the rate again under Pricing brings tax back on every line: 11,200 x 1.13", (await total()) === money(1265600, "USD"), await total());
  ...
  await tabs(/options/i);
  const moneyBtn = page.locator("[data-test=opt-money]").getByRole("button", { name: /money/i });
  if ((await moneyBtn.getAttribute("aria-expanded")) !== "true") await moneyBtn.click();
  ...
  await sendDialog.waitFor({ state: "detached", timeout: 10000 });
  const linkRow = page.locator("aside [data-test=link-row]");
  await linkRow.getByText(/^Live\./).waitFor({ timeout: 10000 });
  const publicUrl = await linkRow.locator("span[title]").getAttribute("title");
  ok("after sending, the dialog closes and the panel shows the live link", /\/p\/[0-9a-f-]{36}$/.test(publicUrl), publicUrl);
