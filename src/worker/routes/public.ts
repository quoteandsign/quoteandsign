...
// Everything under /p/* passes the edge limiter first: a flood from one address is refused before
// it can touch the database at all.
publicRoutes.use("*", async (c, next) => {
  const gate = c.env.BURST;
  if (gate && !(await gate.limit({ key: clientIp(c.req.raw) })).success) return c.text("Too many requests. Try again in a minute.", 429);
  await next();
});

/**
 * The owner and joined team members always; the free-text notify lists only once the plan is paid.
 * Unverified addresses on a fresh trial account would otherwise make a mail relay out of "ask".
 */
async function internalRecipients(db: ReturnType<typeof getDb>, owner: { id: string; email: string; notifyEmails: string[] | null; plan: string; trialEndsAt: Date | null }, proposal: { notifyEmails: string[] | null }): Promise<string[]> {
  ...
  const extras = effectivePlan(owner).paid !== "free" ? [...(owner.notifyEmails ?? []), ...(proposal.notifyEmails ?? [])] : [];
  return [...new Set([owner.email, ...members.map((m) => m.email), ...extras])];
}

/**
 * The unlock cookie proves a password was entered for this proposal, and for this password:
 * changing or removing the password logs every reader out.
 */
async function unlockToken(proposal: { publicId: string; passwordHash: string | null }): Promise<string> {
  return `${proposal.publicId}.${(await sha256Hex(proposal.passwordHash ?? "")).slice(0, 16)}`;
}
async function unlocked(c: { env: { SESSION_SECRET: string }; req: Context<AppEnv>["req"] }, proposal: { publicId: string; passwordHash: string | null }): Promise<boolean> {
  return (await verifyValue(c.env.SESSION_SECRET, getCookie(c as Context<AppEnv>, unlockCookie(proposal.publicId)))) === (await unlockToken(proposal));
}
...
publicRoutes.get("/:publicId", async (c) => {
  ...
  // The edge remembers a reader for a minute at no cost; the database only hears about the first hit.
  if (!isOwner && (!c.env.ONCE || (await c.env.ONCE.limit({ key: `v:${clientIp(c.req.raw)}:${publicId}` })).success)) {
    const now = new Date();
...
publicRoutes.post("/:publicId/unlock", async (c) => {
  ...
  if ((origin && origin !== new URL(c.req.url).origin) || (fetchSite && fetchSite !== "same-origin" && fetchSite !== "none")) return c.redirect(`/p/${publicId}`);
  const loaded = await loadProposal(c, publicId);
...
publicRoutes.post("/:publicId/decline", async (c) => {
  ...
  if (proposal.passwordHash && !(await unlocked(c, proposal))) return c.json({ error: "Unlock the proposal first." }, 401);
  ...
  await sendEmail(c.env, {
    ...
    quote: parsed.data.reason || undefined,
...
publicRoutes.post("/:publicId/ask", async (c) => {
  ...
  const perOwner = await rateLimit(db, `ask:o:${owner.id}`, 60, 24 * 60 * 60_000);
  if (!perViewer.allowed || !perProposal.allowed || !perOwner.allowed) return c.json({ error: "Too many questions for now. Please try again later." }, 429);
...
const engageSchema = z.object({
  sections: z.array(z.object({ id: z.string().regex(/^[\w-]{1,64}$/), title: z.string().trim().transform((s) => s.slice(0, 120)), seconds: z.number().int().min(1).max(120) })).min(1).max(8),
});
...
publicRoutes.post("/:publicId/engage", async (c) => {
  ...
  if (proposal.passwordHash && !(await unlocked(c, proposal))) return c.body(null, 204);
  let body: unknown;
  try {
  ...
  if (c.env.BEACON && !(await c.env.BEACON.limit({ key: clientIp(c.req.raw) })).success) return c.body(null, 204);
  const viewerHash = await ipHash(c.env.SESSION_SECRET, clientIp(c.req.raw));
  const limit = await rateLimit(db, `engage:${publicId}:${viewerHash}`, 400, 60 * 60_000);
  if (!limit.allowed) return c.body(null, 204);
  const now = new Date();
  const titles = new Map<string, string>([["intro", "Introduction"], ["s-pricing", "Pricing"], ["s-accept", "Accept"]]);
  for (const sec of splitSections(proposal.content as Block[])) titles.set(sec.id, sec.title || sec.id);
  const writes = parsed.data.sections
    .filter((s) => titles.has(s.id))
    .map((s) =>
      db
        .insert(schema.sectionTime)
        .values({ proposalId: proposal.id, sectionId: s.id, title: titles.get(s.id)!, seconds: s.seconds, updatedAt: now })
        .onConflictDoUpdate({
  ...
  if (writes.length) await db.batch(writes as [typeof writes[0], ...typeof writes]);
  return c.body(null, 204);
});
